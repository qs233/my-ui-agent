...
4. **构建 VCT**：`buildVisualContainmentTree()` 只消费 `CollapsedNode[]`，输出 `VctNode[]`；顶层入口会把 DOM 记录表、折叠节点索引和 VCT 根节点包装成 `VctSnapshot`。VCT 构建按 DOM-first parent resolution 处理：
   ...
   - **R-Tree 慢路径**：DOM parent 不可用时，通过全量空间索引寻找候选容器，过滤掉不满足近似包含、绘制顺序、固定定位或滚动/裁剪边界规则的节点，再选择面积最小的容器作为视觉父节点。
...
VCT 构建是 DOM-first：优先使用 collapsed DOM parent。只有 DOM parent 不满足 VCT 父节点条件时，才进入 R-Tree 慢路径寻找更合适的空间父节点。R-Tree 在这里只是候选索引，不决定构建顺序。

DOM 快路径和 R-Tree 慢路径共用同一套父节点合法性规则：使用**近似包含**，而不是只接受绝对包含。负边距、定位偏移、尺寸取整等因素可能让子节点轻微越出容器，绝对包含会漏掉这些合理关系。
...
如果 DOM 快路径和 R-Tree 慢路径都找不到合法父节点，非 fixed/sticky 节点会尝试挂回最近的 `maybe-scroll` 边界；这类挂载允许节点完整 bounds 超过边界，并会标记 `floating`。仍找不到时才作为 VCT root。

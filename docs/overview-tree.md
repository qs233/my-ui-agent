...
3. **折叠 DOM wrapper**：`collapseDomTree()` 恢复 retained DOM，后序折叠满足绝对包含和绘制顺序条件的单子 wrapper；preserve tags、`fixed`、`sticky` 和滚动/裁剪候选节点作为边界不参与折叠。输出 parent 已指向存活代表的 `CollapsedTreeNode[]`。

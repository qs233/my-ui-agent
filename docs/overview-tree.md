...
VCT（Visual Containment Tree）用于把网页复杂的 DOM 结构整理成一棵以视觉包含关系为主的树，供 UI Agent 快速理解页面区域、文本、媒体和可能的操作边界。
...
- 折叠安全的单子 DOM wrapper，减少无意义层级；折叠后通过 `wrapperDomIds` 保留被折叠 wrapper 的 DOM 节点 ID。
- 保留可能带来重要视觉、媒体、表单或原生交互语义的标签边界，不为了紧凑输出而吞掉它们。
- 正确处理弹窗、吸顶栏等 `fixed`、`sticky` 或高 `z-index` 元素，避免它们被普通页面容器错误包含。
- 输出应紧凑、可读，并保留理解页面所需的 VCT 引用 ID、标签、类名、名称、文本、可滚动标记和必要的重挂载元数据。
...
3. **折叠 DOM wrapper**：`collapseDomTree()` 恢复 retained DOM，后序折叠满足绝对包含和绘制顺序条件的单子 wrapper，输出 parent 已指向存活代表的扁平 `CollapsedNode[]`。
4. **构建 VCT**：`buildVisualContainmentTree()` 只消费 `CollapsedNode[]`，按面积从大到小处理，输出 `VctNode[]`，并分为两条路径：
...
`CollapsedNode` 与 `VctNode` 分离：前者只表示折叠后的 DOM 节点，后者额外包含 `vctId`、`vctParentId`、`isReparented`、`floating` 和可选 `alignToId`。

## 当前边界

- `RawNode.isInteractive` 仍在准备阶段保留，但 collapse 和序列化阶段不再基于它生成特殊节点类型。
- `RawNode.id` 是当前页面/CDP 会话中的 DOM/backend id；`VctNode.vctId` 是最终缩进文本中的短引用 id。
- `VctNode.vctParentId` 表示视觉树父节点；`domParentId` 保留折叠后 DOM 父节点的 DOM id，用于 reparent 溯源。
...
通过非完全包含挂载的节点会标记 `floating`。当最终 VCT 父节点和 `domParentId` 指向的 DOM 父节点不一致时，节点会标记 `reparented`；序列化时可附带 `dom_parent_id=<vctId>`。`align_to_id` 只对 reparented 节点计算，并通过可替换的 alignment resolver 生成。

...
2. **准备 DOM 记录表**：主路径使用 `visibleNodesFromSnapshot()` 直接从 `SnapshotResponse` 收集元素和文本候选，完成可见性与尺寸过滤、文本归属和最近有效 parent 重写，输出 `DomNodeRecord[]`。这里的 record 只覆盖经过可视筛选保留下来的 DOM 节点，不是原始 CDP snapshot layout 全量节点表。
3. **折叠 DOM wrapper**：`collapseDomTree()` 恢复 retained DOM，后序折叠满足绝对包含和绘制顺序条件的单子 wrapper；preserve tags、`fixed`、`sticky` 作为边界不参与折叠。滚动/裁剪候选节点保留自身为代表节点，但可以吸收内部普通独子 wrapper。输出 parent 已指向存活代表的 `CollapsedTreeNode[]`。
4. **构建 VCT**：`buildVisualContainmentTree()` 只消费 `CollapsedTreeNode[]`，输出 `VctNode[]`；顶层入口会把 DOM 记录表、折叠节点索引和 VCT 根节点包装成 `VctSnapshot`。VCT 构建按 DOM-first parent resolution 处理：
   - **DOM 快路径**：优先尝试使用 collapsed DOM parent。只有同时满足空间包含、绘制顺序、固定定位和滚动/裁剪边界规则时才直接挂载。
   - **DOM 祖先轻量路径**：direct parent 不可用时，最多沿 collapsed DOM 祖先链向上找 5 层，使用最近的合法祖先。
   - **R-Tree 慢路径**：DOM parent 和近祖先都不可用时，通过全量空间索引寻找候选容器，过滤掉不满足近似包含、绘制顺序、固定定位或滚动/裁剪边界规则的节点，再选择面积最小的容器作为视觉父节点。
   - **边界兜底**：慢路径仍找不到时，滚动/裁剪边界内的非 fixed/sticky 节点挂回最近边界并标记 `floating`；否则保留为独立根节点。
...
collapseDomTree(raw) → CollapsedTreeNode[]
buildVisualContainmentTree(collapsed) → VctNode[]
captureOverview(page/url) → VctSnapshot
```

`DomNodeRecord`、`CollapsedNode`、`CollapsedTreeNode` 与 `VctNode` 分离：`DomNodeRecord` 是当前快照里可查询的 retained DOM 记录；`CollapsedNode` 只表示折叠后的 DOM 节点和它关联的 record id；`CollapsedTreeNode` 在此基础上保留折叠后的 DOM 子树；`VctNode` 额外包含 `vctId`、`vctParentId`、`isCollapsed`、`isReparented`、`floating` 和可选 `alignToId`。`VctSnapshot` 是对下游和后续工具函数的顶层数据包，其中 `collapsedNodes` 仍是按 id 查询的扁平 Map。
...
VCT 构建是 DOM-first：优先使用 collapsed DOM parent；如果 direct parent 不满足 VCT 父节点条件，会先沿 collapsed DOM 祖先链最多向上找 5 层，使用最近的合法祖先。只有 direct parent 和近祖先都不可用时，才进入 R-Tree 慢路径寻找更合适的空间父节点。R-Tree 在这里只是候选索引，不决定构建顺序。

DOM 快路径、DOM 祖先轻量路径和 R-Tree 慢路径共用同一套父节点合法性规则：使用**近似包含**，而不是只接受绝对包含。负边距、定位偏移、尺寸取整等因素可能让子节点轻微越出容器，绝对包含会漏掉这些合理关系。
...
- `fixed`、`sticky` 节点不能被普通流式容器吸收，只能挂到 fixed/sticky 候选下或成为独立根。
- 非 fixed/sticky 节点不能跨出最近的 `maybe-scroll` 滚动/裁剪边界重挂载。

R-Tree 慢路径还会额外过滤：

- 候选不能是节点自身。
- 候选不能是该节点的 collapsed-tree 后代，避免形成环。
- 在所有有效候选中选择面积最小、包裹最紧的容器。

绝对包含可以作为候选排序时的高置信信号，但不适合作为慢路径的硬性门槛。

如果 DOM 快路径、DOM 祖先轻量路径和 R-Tree 慢路径都找不到合法父节点，非 fixed/sticky 节点会尝试挂回最近的 `maybe-scroll` 边界；这类挂载允许节点完整 bounds 超过边界，并会标记 `floating`。仍找不到时才作为 VCT root。

通过非完全包含挂载的节点会标记 `floating`。当最终 VCT 父节点和 `ctParentId` 指向的折叠树父节点不一致时，节点会标记 `reparented`；序列化时可附带 `dom_parent_id=<vctId>` 指向折叠树父节点对应的 VCT 节点。只有显式提供 alignment resolver 时，`align_to_id` 才会对 reparented 节点计算并输出。

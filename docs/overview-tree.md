...
4. **构建 VCT**：`buildVisualContainmentTree()` 只消费 `CollapsedNode[]`，按面积从大到小处理，并分为两条路径：
...
buildVisualContainmentTree(collapsed) → TreeNode[]
```

## 当前边界

- `RawNode.isInteractive` 仍在准备阶段保留，但 collapse 和序列化阶段不再基于它生成特殊节点类型。
- 序列化输出直接显示真实标签；只有没有 retained 子节点的节点带 `LEAF` 标记。
